...
public class Agent {
    ...
    public void backtrack(int steps) {
        ...
        // Adjust steps if history is shorter
        if (moveHistory.size() < steps) {
            steps = moveHistory.size();
        }

        // Pop the specified number of positions
        ...
        if (moveHistory.isEmpty()) {
            if (steps == 0) {
                System.out.println("Agent " + id + " has no move history, no backtracking performed");
                return; // Stay in place
            } else {
                // No history left, find random position
                System.out.println("Agent " + id + " has no more history to backtrack to, finding new position");
                int[] newPos = maze.findRandomValidPosition();
                if (newPos != null && isMoveValid(newPos[0], newPos[1])) {
                    int oldX = currentX;
                    int oldY = currentY;
                    currentX = newPos[0];
                    currentY = newPos[1];
        ...
        // Move to the new top of the stack
        ...
        if (isMoveValid(newX, newY)) {
            ...
            maze.updateAgentLocation(this, oldX, oldY);
            backtracks++;
            System.out.println("Agent " + id + " backtracked to (" + newX + "," + currentY + ")");
        } else {
            isStuck = true;
